...
func connectToTor() *net.TCPConn {
	or, err := pt.DialOr(&ptInfo, "", "dummy")
	if err != nil {
		return nil
	}
	return or
...
func main() {
	...
	for _, bindaddr := range ptInfo.Bindaddrs {
		...
		case "dummy":
			...
			server := &http.Server{
				Addr:      fmt.Sprintf(":%s", configs["localport"]),
				TLSConfig: tlsConfig,
			}

			go server.ListenAndServeTLS("Config/server.crt", "Config/server.key")
		default:
